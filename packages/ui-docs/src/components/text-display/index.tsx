...
const transformTextStyle = value => value.replace(' ', '.').replace(' ', '.').toLowerCase();
...
const Header = ({ ...rest }) => (
  <Flex pb={4} alignItems="baseline" borderBottom="1px solid" borderColor="ink.100">
    <Box width={1 / 3}>
  ...
  return (
    <Box pt={5}>
      ...
      {types.map((typeStyle, key) => {
        ...
        return (
          <Flex borderBottom="1px solid" borderColor="ink.100" alignItems="center" py={5} key={key}>
            <Box width={1 / 3}>
              <Box>
